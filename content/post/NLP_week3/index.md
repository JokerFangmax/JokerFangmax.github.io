...
但如果我们想用 attention 完全替代 RNN(***即把QKV那一套用在Encoder和Decoder内部，现在更多的是Decoder的Q到Encoder的KV，而两部分内部的关系实际还是靠RNN的累积更新hidden state***)，仅仅建模“输入和输出之间的关系”还不够，还需要建模：
...
***self-attention中query和value是不是同一个东西***
...
这就是 **information leakage**。PPT 中也明确指出，naive self-attention 对 encoder 没问题，但在 decoder 中会让 next-token prediction 变得 trivial。
...
***补充Mask矩阵应用到Softmax中的公式，同时为什么上面GPT给的矩阵形式和PPT上的不一样***

这样 attention 只能使用当前及历史信息，从而满足自回归生成的因果约束。

***Attention with causal masks is also called unidirectional attention, while non-masked attention is called bidirectional attention.***

***LSTM原理补充https://chatgpt.com/g/g-p-6846e7c64a7881918f7971df0e10640d/c/69c248b0-c818-83a3-997c-3f4012077615***
...
作用是让 decoder 在生成过程中访问源序列信息。PPT 中也强调，正是 self-attention 和 cross-attention 的结合，使得 RNN 可以被完全替代。
...
这种结构天然不适合现代 GPU 的大规模并行。PPT 中也指出，RNN 的 sequential computation prevents parallelization。

### 3.1.2 Transformer 的关键突破

2017 年，Vaswani 等人在论文 *Attention Is All You Need* 中提出：

> 完全使用 attention 来建模序列，不再依赖 RNN。 ***实际上是不是指的不仅仅是cross attention，更重要的是self attention，否则也可以继续用RNN***
...
* 两个tricks
  * Residual Connection
  * Layer Normalization ***为什么正则化可以避免梯度消失***

***把前面Seq2Seq演变把RNN替换成Attention的简略图和Transformers的图对应比较，并说说Decoder和Encoder有什么不同***
...
- Query：当前 token 想找什么
- Key：当前 token 提供什么索引信息(***可以理解为计算与Query token的相似度用的***)
...
前馈网络由两个线性层组成，两层之间有个激活函数。一般来说第一层是上采样层，第二层是下采样层。
...
作用是让信息与梯度更容易传播，减轻深层网络训练困难。 ***补充原理分析为什么***

***另外这是Post-LN，什么是Pre-LN***

#### 3.3.5.2 Layer Normalization

LayerNorm 会在特征维度上做归一化，使训练更稳定。  
原始 Transformer 中，attention 和 FFN 两个子层都配有残差连接和 layer normalization。
...
Decoder layer 和 encoder 类似，但有两个关键变化。
...
然后通过线性层(***一般来说这个线性层是不是和embedding层的参数共享***)映射到词表维度，再接 softmax，得到下一个 token 的概率分布：
...
***有个问题为什么RNN的Encoder/Decoder block都只需要一个，而Transformers中却需要重复好几个block***
...
PPT 中也提到，Transformer 在机器翻译上优于先前架构，同时训练 FLOPs 更少。
...
也就是说，Transformer 虽然强大，但训练并不总是稳定，而且对结构改动比较敏感。

此外，标准 self-attention 的时间和空间复杂度通常是序列长度 $n$ 的平方级： ***为什么是O(N^2)***
$
O(n^2)
$
...
传统位置编码通常是把**绝对**位置向量加到 token embedding 上。  
RoPE 的思路不同：

> 把**相对**位置信息直接融入 Query 和 Key 的变换中。

***补充具体实现细节，具体公式为什么这么设计的***
***RoPE的长文本生成质量问题以及解决方法***

PPT 中列出了 relative position encoding、RoPE、高维 attention 中的 generalized RoPE，以及 RoPE 的成功应用。
...
PPT 中提到 GLU 及其变体能改进 Transformer。
...
***怎么理解上面的公式，和ppt好像不一样？另外引入Wgate要保持训练参数数量而减少维度是怎么减少的***
***GLU为什么效果会比较好，怎么好法***
...
PPT 中指出，原始 Transformer 使用的是 **Post-LN**，而后续研究发现 **Pre-LN** 往往更稳定。 ***插入示意图***
...
RMSNorm 是对 LayerNorm 的进一步简化。PPT 中也提到了 Improved Normalization 与 RMSNorm。
